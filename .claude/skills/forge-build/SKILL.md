...
Where these commands show a `.harness/cargo` prefix, that repo is a **substrate**
consumer: builds there run in a flat-sibling worktree and take a per-lane lock.

Enrolled does not mean wrapped. Hybrid and customer consumers are enrolled — they
carry gates, fences and a `.harness/plan.json` — and they deliberately have no
wrapper (`harnesses/hybrid/harness.yaml` says so where the imports are declared),
which is why their commands below show a bare `cargo`. The PATH shim knows: it asks
`resolve-lane`, reads `wrap: false`, and execs the real cargo. `forge-cli` is one of
these, and a reader who goes looking for a `.harness/cargo` there will not find one.
The only repo with no `.harness/` directory at all is `forge-platform-e2e`
(GitHub-archived, read-only), which has no gates either; run `cargo` directly there.

If a command is refused, the denial names the repair; don't reach for
`.harness/escapes.json` before reading it, and report a refusal that looks wrong
