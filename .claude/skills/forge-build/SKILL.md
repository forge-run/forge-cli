...
builds there run in a flat-sibling worktree and take a per-lane lock. The one repo
without a `.harness/` directory — `forge-platform-e2e` (GitHub-archived, read-only) —
has no wrapper and no fence; run `cargo` directly there. `forge-cli` IS enrolled (12
bound gates, `cargo` wrapper — see its block below); this line said otherwise until
2026-08-28, which is exactly the drift the record-staleness check now watches for. If a command is refused, the denial names the repair; don't
