...
enum Cmd {
    ...
    /// Today that is the dialect, in any of its three spellings: every
    /// `domains/<d>/services/*.py`, `*.ts` and `*.java` goes through its own
    /// front end with the workspace's table schemas loaded, and any refusal
    /// exits non-zero. `--format json` prints the versioned diagnostics
    /// envelope.
